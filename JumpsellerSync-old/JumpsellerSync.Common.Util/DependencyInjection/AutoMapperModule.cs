...
namespace JumpsellerSync.Common.Util.DependencyInjection
{
    public class AutoMapperModule : Autofac.Module
    {
        ...
        protected override void Load(ContainerBuilder builder)
        {
            ...
            var types = openTypes.SelectMany(
                openType => allTypes.Where(
                    t => t.IsClass && !t.IsAbstract &&
                        ImplementsGenericInterface(t.AsType(), openType)));
            foreach (var type in types)
            {
                builder
                   .RegisterType(type.AsType())
            ...
            builder
                .Register<IConfigurationProvider>(
                    ctx => new MapperConfiguration(
                        cfg => cfg.AddMaps(assembliesToScan)))
                .SingleInstance();

            builder
                .Register<IMapper>(
                    ctx => new Mapper(
                        ctx.Resolve<IConfigurationProvider>(), ctx.Resolve))
                .InstancePerDependency();
        }
        ...
        private static bool ImplementsGenericInterface(Type type, Type interfaceType)
            => IsGenericType(type, interfaceType) ||
                type.GetTypeInfo().ImplementedInterfaces.Any(
                    @interface => IsGenericType(@interface, interfaceType));
