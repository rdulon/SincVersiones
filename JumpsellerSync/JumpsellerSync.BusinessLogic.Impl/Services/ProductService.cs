          ...
          IProductService
    {
        ...
        public override async Task<ServiceResult<ProductDetailsDto>> CreateAsync(CreateProductDto data)
        {
            ...
            if (providerResult.OperationSucceed)
            {
                ...
                var syncJSResult = await vtexService.SynchronizeProductAsync(product.Id);
        ...
        public async Task<PageResultDto<ProductDetailsDto>> ReadAsync(SearhProductsDto search)
        {
            ...
            try
            {
                var productsResult = await productRepository.ReadAsync(
                    search.SkuOrName, search.BrandIds, offset, search.Limit, search.Provider);
                var list = await productsResult.ToListAsync();
                ...
                pages = await productRepository.GetProductPagesAsync(
                    search.SkuOrName, search.BrandIds, search.Limit, search.Provider);
            }
            catch (Exception e)
            { logger.LogException(e, header); }
