...

    {
        ...
        protected virtual async Task<SynchronizationInfoDto> GetSyncInfoAsync(string logHeader)
        {
            ...
            if (updated)
            {
                ...
                products = await productRepository.ReadSynchronizingToJumpsellerAsync().ToListAsync();
        ...
        protected async Task ClearProducts(List<string> productCodes)
        {
            if (!productCodes.IsNullOrEmpty())
            {
                var productsToUnsync = await productRepository.ReadNotIn(productCodes).ToListAsync();

                foreach (var productToUnsync in productsToUnsync)
                {
                    productToUnsync.Stock = 0;
                    await productRepository.UpdateAsync(productToUnsync);
                }
